...
class Position(namedtuple("Position", "board score wc bc ep kp")):
    '''
    board -- 120char board representation
    score -- evaluation
    wc    -- white castling rights
    bc    -- black castling rights
    ep    -- en passant square
    kp    -- king passant square
    '''
    def gen_moves(self):
        #ray casting for sliding pieces
        #simple map generation for knight/king/pawn
        for i, p in enumerate(self.board):
            ...
            for d in directions[p]:
                for j in count(i + d, d):
                    q = self.board[j]
                    #stay inside board and off friendly pieces
                    if q.isspace() or q.isupper():
                        break
                    #pawn double move and pawn capture
                    if p == "P":
...
class Searcher:
    ...
    def bound(self, pos, gamma, depth, can_null=True):
        '''
        let s* be the true score of subtree
        method returns r, where:
            if gamma > s* then s* <= r < gamma (improved upper bound)
            if gamma <= s* then gamma <= r <= s* (improved lower bound)
        '''
        self.nodes += 1
